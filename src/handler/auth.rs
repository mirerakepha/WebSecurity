use actix_web::{
    cookie::time::Duration as ActixWebDuration, cookie::Cookie, web, HttpResponse, Scope, Responder
};
use serde_json::json;
...
#[utoipa::path(
    ...
    request_body(content = RegisterUserDto, description = "Credentials to create an account", example = json!({
        "email": "johndoe@example.com", "name": "John Doe", "password": "password123", "passwordConfirm": "password123"
    })),
    responses(
...
pub async fn register(
    ...
    Err(sqlx::Error::Database(db_err)) => {
        if db_err.is_unique_violation(){
            Err(HttpError::unique_constraint_violation(ErrorMessage::EmailExist))
        } else {
...
#[utoipa::path(
    ...
    path = "/api/auth/login",
    tag = "Login Endpoint",
    request_body(content = LoginUserDto, description = "Credentials to login into your account", example = json!({"email": "johndoe@example.com", "password": "password123"})),
    responses(
...
#[utoipa::path(
    ...
    tag = "Logout Endpoint",
    responses(
        (status=200, description= "Login successfull"),
        (status=400, description= "Validation Error", body= Response),
        (status=401, description= "Unauthorized Error", body= Response),
        (status=500, description= "Internal Server Error", body= Response),
    ),
    security(("token" = []))
)]
